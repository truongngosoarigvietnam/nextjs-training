<template>
    <div v-show="isShowEdit" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-10">
        ...
        >
            <!-- Modal header -->
            <h2 class="text-lg font-bold mb-4 text-center">Chỉnh sửa Cơ sỡ</h2>
            <!-- Modal body -->
            <form @submit.prevent="handleSave" class="text-start">
                ...
                <div class="text-start">
                    <imageFieldSpecial v-model="image" @change="previewImage2" v-bind:preview="preview" />
                </div>
...
import useClinic from '@/services/editClinic';
import { ref, inject } from 'vue';
import {useLoading} from 'vue-loading-overlay'
import imageFieldSpecial from '../../manager-specialy/modal/imageFieldSpecial.vue';
...
export default {
    setup() {
        ...
        const { handleEditClinic } = useClinic();
        ...
        async function handleSave() {
            ...
            const res = await handleEditClinic({
                id :clinicId.value ,
                name: name.value,
        ...
        emitter.on('showEditModalClinic', (item) => {
            // *Listen* for event
