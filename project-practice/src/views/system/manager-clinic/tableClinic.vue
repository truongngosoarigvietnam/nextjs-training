<template>
    <div class="pt-[3%] flex justify-center h-[100px]">
        ...
        <p class="text-xl font-black text-[#0071BA]">DANH SÁCH CÁC CƠ SỠ</p>
    </div>
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr
                    ...
                    :class="{
                        'bg-[#FAEBD7] hover:bg-[#FAEBD7] ': clinicActive === item.id,
                        'bg-white hover:bg-gray-50 dark:hover:bg-gray-600 ': clinicActive !== item.id,
                    }"
                >
...
import useEventClinic from '@/services/createClinic';
import createClinic from './modal/createClinic.vue';
import editClinic from './modal/editClinic.vue';
export default {
    setup() {
        const {listData , fetchListClinic ,totalPage } = useClinic();
        const { handleDeleteClinic } = useEventClinic();
        const pageIndex = ref(0);
        const clinicActive = ref('');
        ...
        fetchListClinic({ pageIndex: pageIndex.value });
        function nextPage() {
        ...
        const handleCreate = () => {
            emitter.emit('eventShowModalClinic');
        };
        const handleMount = (id) => {
            clinicActive.value = id;
           emitter.emit('eventDoctorByClinic', id);
        };
        emitter.on('eventHandleClinic', () => {
            // *Listen* for event
            fetchListClinic({
                pageIndex: pageIndex.value,
            });
        ...
        const deleteClinic = async (id) => {
            swal.fire({
                ...
                if (result.isConfirmed) {
                    const res = await handleDeleteClinic(id);
        ...
        function handleEdit(item) {
            emitter.emit('showEditModalClinic', item);
