...
export default function Index({ isOpen, setOpen, isDataEdit }: Iprop) {
    ...
    const userCreateSpecial = async (data: FormData): Promise<any> => {
        setIsLoading(true);
        return await api.post(apiRouters.CREATE_SPECIAL, data);
    };
    const { mutate: userCreateSpecialRequest, isLoading } = useMutation('userCreateSpecialRequest', userCreateSpecial, {
        onSuccess: async () => {
            await queryClient.refetchQueries(['getListSpecial']);
    ...
    const userEditSpecial = async (data: FormData): Promise<any> => {
        setIsLoading(true);
        return await api.post(apiRouters.EDIT_SPECIAL, data);
    };
    const { mutate: userEditSpecialRequest } = useMutation('userEditSpecialRequest', userEditSpecial, {
        onSuccess: async () => {
    ...
    const onSubmit: SubmitHandler<IDataCreateClinic> = (data) => {
        ...
        if (isDataEdit) {
            ...
            userEditSpecialRequest(newData);
        } else {
            userCreateSpecialRequest(newData);
        }
    };
    const handleClose = () => {
        reset();
